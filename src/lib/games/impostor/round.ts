...
export function selectRoundWord(
	...
	if (selectedCategories.length !== requestedCategoryIds.size) {
		...
		const missingCategoryIds = selectedCategoryIds.filter((categoryId) => !availableCategoryIds.has(categoryId));
...
function selectStartingPlayer(players: ImpostorPlayer[], random: RandomNumberGenerator): ImpostorPlayer {
...
function randomInteger(min: number, max: number, random: RandomNumberGenerator): number {
	...
	if (!Number.isFinite(value) || value < 0 || value >= 1) {
		throw new Error("Random generator must return a finite number between 0 (inclusive) and 1 (exclusive).");
	}
