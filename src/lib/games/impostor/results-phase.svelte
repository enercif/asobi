<script lang="ts">
	...
	let endReasonLabel = $derived(result.endedReason === "timer" ? "Zeit abgelaufen" : "Runde beendet");
</script>

<div class="flex flex-1 flex-col gap-8 rounded-[2rem] bg-white px-8 py-10 text-center shadow-sm">
	<div class="flex flex-col gap-3">
		<p class="text-sm font-semibold uppercase tracking-[0.35em] text-primary">{endReasonLabel}</p>
		<h2 class="text-4xl font-bold text-black text-balance">Impostors aufgedeckt</h2>
	...
	<div class="flex flex-col gap-4 rounded-3xl bg-black/[0.03] px-6 py-6">
		<p class="text-sm font-semibold uppercase tracking-[0.3em] text-black/45">
	...
	<div class="grid gap-3 text-left sm:grid-cols-2">
		<div class="rounded-2xl bg-black/[0.03] px-5 py-4">
			<p class="text-sm font-semibold uppercase tracking-[0.25em] text-black/45">Kategorie</p>
			<p class="mt-2 text-lg font-semibold text-black">{result.secretWord.categoryName}</p>
		</div>
		<div class="rounded-2xl bg-black/[0.03] px-5 py-4">
			<p class="text-sm font-semibold uppercase tracking-[0.25em] text-black/45">Startspieler</p>
			<p class="mt-2 text-lg font-semibold text-black">{startingPlayer?.name ?? "Unbekannt"}</p>
		</div>
	</div>
